...
//    else anaEL.LoadAnalysisInfo("MassAnalysis", "NbrJetSmearMC", nbrJetSmear);
  ...
  massSolver= new MassSolver();
   ...
   //PDF initialization
  char * tools;
  tools = getenv ("TOOLSDIR");

  char name[300];
  sprintf(name,"%s/lhapdf585_64/share/lhapdf/PDFsets/cteq61.LHgrid",tools);
//  string lhapdfFile(tools+"/lhapdf585_64/share/lhapdf/PDFsets/cteq61.LHgrid");
  cout << "LHAPDF file "<<name<<endl;
  fexists(name, true);
  LHAPDF::initPDFSet(name);
  ...
  sprintf(name,"%s/JR_Standalone/JetMETObjects/data/",tools);
  string JR_Standalone_Path(name);
...
//       double sum_weight_1 = 0;
	...
	sum_weight += s_weight;
	if (verbosity >= 3) cout << " - " << s_weight<< " - " <<sum_weight<<endl;
...
//   cout << "Final in histo: "<<h_temp_MassWeight->GetMaximumBin()<< " "
//   << h_temp_MassWeight->GetBinContent(h_temp_MassWeight->GetMaximumBin())<< " "
//   <<h_temp_MassWeight->GetXaxis()->GetBinLowEdge(h_temp_MassWeight->GetMaximumBin())<< " "
//   <<endl;
